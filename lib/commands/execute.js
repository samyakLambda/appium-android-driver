...
extensions.execute = async function execute(script, args) {
  ...
  const endpoint =
    this.chromedriver.jwproxy.downstreamProtocol === PROTOCOLS.MJSONWP
      ? '/execute'
      : '/execute/sync';
      console.log("this.chromedriver.useFallbackProxy: ", this.chromedriver.useFallbackProxy, " this.chromedriver.fallbackJwProxy: ", this.chromedriver.fallbackJwProxy.sessionId);
      try {
        ...
        return await this.chromedriver.fallbackJwProxy.command(endpoint, 'POST', {
          script,
          args,
        });
      }
